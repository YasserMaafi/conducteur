...
if ($unit === 'wagons' && !is_numeric($quantity) || (int)$quantity != $quantity) {
    throw new Exception("Le nombre de wagons doit être un entier.");
       ...
       // Determine values for quantity and wagon_count based on unit
        ...
        $metadata = [
            'sender' => $sender_client['company_name'], // Use sender's company name
            'sender_id' => $sender_client['client_id'], // Use sender's client ID
            'recipient' => $_POST['recipient_name'],    // Use recipient's name from form
            'origin' => $origin,
                        ...
                        <form method="POST" class="needs-validation" novalidate>
                            ...
                            <div class="form-section">
                                ...
                                <div class="row g-3">
                                    ...
                                    <!-- Hidden input for sender_client_id is not needed as we use the session value -->
                                </div>
                            </div>
...
<small class="form-text text-muted">Entrez soit le poids total en kg ou le nombre de wagons.</small>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const departSelect = document.querySelector('select[name="gare_depart"]');
            const arriveeSelect = document.querySelector('select[name="gare_arrivee"]');
                ...
                if (departSelect.value && arriveeSelect.value) {
                    ...
                    mapContainer.innerHTML = `
                        <div class="text-center py-4">
                            ...
                            <h5>Trajet: ${departSelect.options[departSelect.selectedIndex].text} 
                                → ${arriveeSelect.options[arriveeSelect.selectedIndex].text}</h5>
                            <p class="text-muted">Visualisation du trajet entre les gares sélectionnées</p>
                        </div>
                    `;
                }
            }
            
            departSelect.addEventListener('change', updateMapPreview);
            arriveeSelect.addEventListener('change', updateMapPreview);
        });
    </script>
</body>
</html>